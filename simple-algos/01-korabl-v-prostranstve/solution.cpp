...
struct QueueWithMax {
  ...
  Cell GetMax() const { return deq.front(); }
...
void BuildPaths(Grid3DPos& answers, const Grid3DCell& max_xyz_pass,
                const Grid3DInt& grid) {
...
void ProcessQueries(const Grid3DPos& answers) {
  ...
  for (int query = 0; query < queries; ++query) {
    int x, y, z;
    std::cin >> x >> y >> z;
    const Position& ans = answers[x][y][z];
...
int main() {
  ...
  Grid3DInt grid(size_x,
                 std::vector<std::vector<int>>(size_y, std::vector<int>(size_z)));

  Cell default_cell = {0, 0, 0, 0};
  Grid3DCell max_z_pass(
      size_x, std::vector<std::vector<Cell>>(size_y,
                                             std::vector<Cell>(size_z, default_cell)));
  Grid3DCell max_yz_pass(
      size_x, std::vector<std::vector<Cell>>(size_y,
                                             std::vector<Cell>(size_z, default_cell)));
  Grid3DCell max_xyz_pass(
      size_x, std::vector<std::vector<Cell>>(size_y,
                                             std::vector<Cell>(size_z, default_cell)));

  Grid3DPos answers(
      size_x, std::vector<std::vector<Position>>(size_y, std::vector<Position>(size_z)));
